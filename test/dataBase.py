...
def get_db():
    ...
    if not db:
        db = g._database = sqlite3.connect(DATA_BASE_PATH)
        db.row_factory = sqlite3.Row
    return db


def init_users_table(app):
    with app.app_context():  # Works inside app even when it's not running
        db = get_db()
        cursor = db.cursor()
        cursor.execute(
            '''
